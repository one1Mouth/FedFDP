...
from .fedoptimizer import PerturbedGradientDescent, SCAFFOLDOptimizer
from .utils.dp_utils import cartesian_to_polar, polar_to_cartesian, vector_to_matrix, cartesian_add_noise, \
    devide_epslion, apply_dp_sgd_analysis
...
def make_optimizer_class(cls):
    class DPOptimizerClass(cls):
        ...
        # 单样本梯度裁剪（每轮固定范数C）
        def microbatch_step(self, is_privacy=True, is_auto_s=False, is_fairness=False, fair_coef=1.0):  # 一个样本
            """
            is_privacy：用这个优化器的都会True
            is_auto_s: Ture的话就用AUTO-S的正则化裁剪，否则用普通的Abadi裁剪
            《Automatic Clipping: Differentially Private Deep Learning Made Easier and Stronger》
            is_fairness：这个是FedPRF里的内容，跟公平性有关
            fair_coef：is_fairness=True的配套参数
            """
            total_norm = 0.
            ...
            clip_coef = 1.0  # 不裁剪
            if is_privacy:  # 如果要进行DP
                if is_auto_s:  # 如果使用AUTO-S正则化裁剪
                    clip_coef = (1.0 / (total_norm + 1e-2))  # 正则化裁剪的裁剪值，把fair_coef分子分母约掉了
                else:  # 用正常的Abadi裁剪
                    if not is_fairness:  # 如果不需要公平的话，就传统的方法
                        clip_coef = min(self.l2_norm_clip / (total_norm + 1e-6), 1.)  # 范数比较，得到等下要裁剪的部分
                    else:  # 需要公平的话，动态裁剪
                        clip_coef = fair_coef * min(self.l2_norm_clip / (fair_coef * total_norm + 1e-6), 1.)
        ...
        # 单样本梯度裁剪（每轮固定范数C）
        def microbatch_step_perturbed(self, mu, global_params, device, is_privacy=True, is_auto_s=False):  # 一个样本
            """
            is_privacy：用这个优化器的都会True
            is_auto_s: Ture的话就用AUTO-S的正则化裁剪，否则用普通的Abadi裁剪
            《Automatic Clipping: Differentially Private Deep Learning Made Easier and Stronger》
            ...
            # 范数的计算,遍历一个样本梯度中每个元素
            for group in self.param_groups:  # 整个group里面包含params和accum_grads两个模块，params是每层的张量（正常单元和偏执单元分开）
                for param, global_param in zip(group['params'], global_params):  # param是具体的那个张量（逐层），偏执单元和正常单元的张量分开
                    if param.requires_grad:
                        global_param = global_param.to(device)
                        new_grad = param.grad.data + mu * (param.data - global_param.data)
                        total_norm += new_grad.data.norm(2).item() ** 2.  # 对每层求范数然后把根号开出来，然后对它们求和
        ...
        # 单样本梯度裁剪（每轮固定范数C）
        def microbatch_step_q_FFL(self, is_privacy=True, is_auto_s=False, is_qFFL=True,
                                  f_loss=1.0, q_qFFL=1.0, grad_l2_norm=1.0, ordinary_learning_rate=0.1):  # 一个样本
            """
            这个是用来给qFFL做梯度下降的
            """
            total_norm = 0.
            ...
            clip_coef = 1.0  # 不裁剪
            loss_power_q = math.pow(f_loss + 1e-2, q_qFFL)
            if is_privacy:  # 如果要进行DP
                if is_auto_s:  # 如果使用AUTO-S正则化裁剪
                    clip_coef = (1.0 / (total_norm + 1e-2))  # 正则化裁剪的话，F^q约掉了
                else:  # 用正常的Abadi裁剪，注意梯度是 F^q * grad F
                    clip_coef = loss_power_q * min(
                        self.l2_norm_clip / (loss_power_q * total_norm + 1e-6), 1.)  # 范数比较，得到等下要裁剪的部分
            if is_qFFL:
                h_kt = q_qFFL * math.pow(f_loss + 1e-2, q_qFFL - 1) * math.pow(grad_l2_norm + 1e-2, 2) \
                       + (1 / ordinary_learning_rate) * math.pow(f_loss + 1e-2, q_qFFL)
                clip_coef = clip_coef / h_kt  # 动态学习率在这里乘掉，外面optimizer的学习率置为1.0
            # 梯度的裁剪
            for group in self.param_groups:
                for param, accum_grad in zip(group['params'], group['accum_grads']):
                    #   print("accum_grad:", accum_grad.shape)
        ...
        def step_dp_scaffold(self, server_cs, client_cs, *args, **kwargs):
            for group in self.param_groups:
                # print("group: ",group)
                for param, accum_grad in zip(group['params'],
                                             group['accum_grads']):  # 整个group里面包含params和accum_grads两个模块 逐层操作
            ...
            # 调用原函数的梯度下降,假设这个step只做梯度下降，包括学习率的那个梯度更新操作
            super(DPOptimizerClass, self).step(server_cs, client_cs, *args, **kwargs)

        def step_dp_perturbed(self, global_params, device, *args, **kwargs):
            for group in self.param_groups:
                # print("group: ",group)
                for param, accum_grad in zip(group['params'],
                                             group['accum_grads']):  # 整个group里面包含params和accum_grads两个模块 逐层操作
            ...
            # 调用原函数的梯度下降,假设这个step只做梯度下降，包括学习率的那个梯度更新操作
            super(DPOptimizerClass, self).step(global_params, device, *args, **kwargs)
        ...
        def microbatch_step_perlayer(self):
            ...
            for i in range(0, 8):
                clip_coef_perlayer[i] = min(self.l2_norm_clip[i] / (perlayer_norm[i] + 1e-6),
                                            1.)  # 范数比较，得到等下每层要裁剪的部分，这边每层裁剪的范数已经在上一轮保存在l2_norm_clip中
            ...
            for group in self.param_groups:
                ...
                for param, accum_grad in zip(group['params'],
                    ...
                    if param.requires_grad:
                        ...
                        param.grad.data.add_(
                            self.l2_norm_clip[i] * np.sqrt(len(group)) * self.noise_multiplier * torch.randn_like(
                                param.grad.data))  # 每层分别加噪
...
DPSGD_perturbed = make_optimizer_class(PerturbedGradientDescent)
DPSGD_SCAFFLOD = make_optimizer_class(SCAFFOLDOptimizer)
